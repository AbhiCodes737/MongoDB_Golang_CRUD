...
var userCollection = db.Db().Database(<database>).Collection(<collection_name>) // mongodb client instance

func CreateUser(w http.ResponseWriter, r *http.Request) {
	var person models.User                         // variable of type User in mongodb
	err := json.NewDecoder(r.Body).Decode(&person) // decodes body and enter to person variable
	if err != nil {
		fmt.Print(err)
	}
